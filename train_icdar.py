...
from utils import data_reader_icdar, dataset_text
...
def train(checkpoint=None):
    ...
    img_dir = 'data/icdar2015/train'
    data_source = data_reader_icdar.DataReader(img_dir, config=configs)
    ...
    learning_rate = tf.train.piecewise_constant(global_step, [8000, 15000], [0.0002, 0.00005, 0.00001])
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, allow_soft_placement=True)) as sess:
        ...
        for epoch in range(1, 1+configs.epochs):
            ...
            if epoch % 10 == 0:
                saver.save(sess, "./checkpoint/icdar/weight50_icdar", global_step=global_step)
