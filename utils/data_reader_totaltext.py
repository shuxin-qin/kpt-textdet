...
class DataReader(object):
    ...
    def __getitem__(self, idx):
        ...
        #print(img_id)
        txt_fn = os.path.join(self.img_dir, 'labels', 'poly_gt_' + img_id.replace('.jpg','.txt'))
    ...
    def load_annoatation_txt(self, txt_file):
        ...
        with open(txt_file, 'r', encoding='utf-8-sig') as f:
            for line in f.readlines():
                ...
                tline_list = split_line[-1].strip().split('\'')
                if len(tline_list) > 1:
                    tline = split_line[-1].strip().split('\'')[1]
                else:
                    tline = ''
                ...
                if len(xs) < 4 or len(ys) < 4 or len(xs) != len(ys):
                    continue
                ...
                thi = []
                lp = len(xs)
                for i in range(int(lp/2)):
                    cpts.append([(xs[i]+xs[lp-1-i])/2, (ys[i]+ys[lp-1-i])/2])
                    thi.append(math.sqrt(((xs[i]-xs[lp-1-i])**2)+((ys[i]-ys[lp-1-i])**2)))
                #2、计算kpt, thicks
                kpt, thick = self.cal_kpts(cpts, thi)
    ...
    def gen_kernals(self, polygons, shape):

        gt_text = np.zeros(shape, dtype='uint8')
        for i in range(polygons.shape[0]):
            cv2.drawContours(gt_text, [polygons[i]], -1, 1, -1)

        #gt_center = np.zeros(shape, dtype='uint8')
        kernal_bboxes = self.shrink(polygons, 0.4)
        for i in range(kernal_bboxes.shape[0]):
            cv2.drawContours(gt_text, [kernal_bboxes[i]], -1, 10, -1)
    ...
    def shrink(self, bboxes, rate, max_shr=1000):
        rate = rate * rate
        shrinked_bboxes = []
        for bbox in bboxes:
