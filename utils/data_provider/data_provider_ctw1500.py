    ...
    while True:
        ...
        for i in index:
            try:
                ...
                # distance transform
                #dis_trans = distence_transform(text_polys, im.shape[0:2])
                ...
                for i in range(0, kernel_num-1):
                    ...
                    kernal_bboxes = shrink(text_polys, rate)
                    for i in range(text_polys.shape[0]):
                        cv2.drawContours(gt_kernal, [kernal_bboxes[i]], -1, 1, -1)
                    gt_kernals.append(gt_kernal)
                
                #print('time:', time.time() - start)
                #kernel_num = scale_ratio.shape[0] - 1
                #gt_kernals = gen_kernels(text_polys, im.shape[0:2], kernel_num)

                imgs = [im, gt_text, training_mask]
                ...
                #dis_trans_masks.append(dis_trans[:, :, np.newaxis].astype(np.float32))
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        cv2.imshow('test0', seg_map[:,:,0].astype('uint8')*255)
        '''
        cv2.namedWindow("test1",0)
        cv2.resizeWindow("test1", 512, 512)
        cv2.imshow('test1', seg_map[:,:,1].astype('uint8')*255)
        cv2.namedWindow("test2",0)
        cv2.resizeWindow("test2", 512, 512)
        cv2.imshow('test2', seg_map[:,:,2].astype('uint8')*255)
        cv2.namedWindow("test3",0)
        cv2.resizeWindow("test3", 512, 512)
        cv2.imshow('test3', seg_map[:,:,3].astype('uint8')*255)
        cv2.namedWindow("test4",0)
        cv2.resizeWindow("test4", 512, 512)
        cv2.imshow('test4', seg_map[:,:,4].astype('uint8')*255)
        
        cv2.namedWindow("test5",0)
        cv2.resizeWindow("test5", 512, 512)
        cv2.imshow('test5', seg_map[:,:,5].astype('uint8')*255)
        '''
