...
def scale_wh(image, bboxes, kpts, thicks, size):
    
    h0, w0 = size
    h, w, _ = image.shape
    factor = min(w0*1.0/w, h0*1.0/h)
    image = cv2.resize(image, (0, 0), fx=factor, fy=factor, interpolation=cv2.INTER_CUBIC)
    bboxes = bboxes * factor
    kpts = kpts * factor
    thicks = thicks * factor

    return image, bboxes, kpts, thicks
...
def random_scale(image, bboxes, kpts, thicks):
...
def random_rotate(image, bboxes, kpts, thicks, angle=30, scale=1):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        '''
        kpt = kpts[i]
        kp1 = np.dot(rot_mat, np.array([kpt[0][0], kpt[0][1], 1]))
        kp2 = np.dot(rot_mat, np.array([kpt[1][0], kpt[1][1], 1]))
        kp3 = np.dot(rot_mat, np.array([kpt[2][0], kpt[2][1], 1]))
        kp4 = np.dot(rot_mat, np.array([kpt[3][0], kpt[3][1], 1]))
        kp5 = np.dot(rot_mat, np.array([kpt[4][0], kpt[4][1], 1]))
        kp6 = np.dot(rot_mat, np.array([kpt[5][0], kpt[5][1], 1]))
        kp7 = np.dot(rot_mat, np.array([kpt[6][0], kpt[6][1], 1]))
        concat = np.vstack((kp1,kp2,kp3,kp4,kp5,kp6,kp7))
        concat = concat.astype(np.int32)
        rot_kpts.append(concat)

        kp_wid = thicks[i]
        pplist = cal_width([kp1,kp2,kp3,kp4,kp5,kp6,kp7], kp_wid)
        rx, ry, rw, rh = cv2.boundingRect(np.vstack(pplist))
        rx_min = rx
        ry_min = ry
        rx_max = rx+rw
        ry_max = ry+rh
