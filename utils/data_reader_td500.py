...
class DataReader(object):
    def __init__(self, img_dir, config=None):
        ...
        self.images = self.get_img_list(self.img_dir, exts=['JPG'])
    ...
    def __getitem__(self, idx):
        ...
        txt_fn = os.path.join(self.img_dir, img_id.replace('.JPG','.gt'))
        ...
        if polys.shape[0] > 0:
            ...
            kpts[..., 0] = np.clip(kpts[..., 0], 0, org_w)
            kpts[..., 1] = np.clip(kpts[..., 1], 0, org_h)
        ...
        gt_t = np.where(gt_text>=0.01, 100, 0)
    ...
    def load_annoatation_txt(self, txt_file):
        ...
        with open(txt_file, 'r', encoding='utf-8-sig') as f:
            for line in f.readlines():
                ...
                x1, y1, x2, y2, x3, y3, x4, y4 = self.rec_rotate(x, y, w, h, r)
    ...
    def xy_rorate(self, theta, x, y, centerx, centery):
        """
        针对中心点进行旋转
        ...
        :return:
        """
        r_x, r_y = self.rotate(theta, x - centerx, y - centery)
        return centerx+r_x, centery+r_y


    def rec_rotate(self, x, y, width, height, theta):
        """
        ...
        x1, y1 = self.xy_rorate(theta, x, y, centerx, centery)
        x2, y2 = self.xy_rorate(theta, x+width, y, centerx, centery)
        x3, y3 = self.xy_rorate(theta, x, y+height, centerx, centery)
        x4, y4 = self.xy_rorate(theta, x+width, y+height, centerx, centery)
