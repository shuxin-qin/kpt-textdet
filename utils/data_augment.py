...
class DataAugmentForObjDet():
    ...
    def _change_contrast_brightness(self, img):
        #g(x) = αf(x) + β，其中：α(>0)、β常称为增益与偏置值，分别控制图片的对比度和亮度  [0.4, 80][0.7,40] [1.4, -45] [1.8, -90]
        a = [0.4, 0.7, 1.5, 2.0]
        b = [80, 40, -60, -120]
        index = np.random.randint(4)
        res = np.uint8(np.clip((a[index] * img + b[index]), 0, 255))
        return res
    ...
    # 旋转
    def _rotate_img_bbox(self, img, bboxes, angle=30, scale=1.):
        '''
    ...
    # 缩放
    def _resize_img_bboxes(self, img, bboxes, w, h):
        ...
        img_out = cv2.resize(img, (w, h), interpolation=cv2.INTER_CUBIC)

        scale_bboxes = list()
        for bbox in bboxes:
            scale_bboxes.append([int(bbox[0]*ws), int(bbox[1]*hs), int(bbox[2]*ws), int(bbox[3]*hs)])

        return img_out, scale_bboxes


    def data_augment(self, img, bboxes):

        flag = np.random.randint(self.num)
        if flag == 0:
            #print('裁剪')
            ...
            #img = self._change_contrast_brightness(img)
        elif flag == 3:
            #print('翻转')
            horizon = True
            #if np.random.randint(2) == 0:
            #    horizon = False
            img, bboxes = self._filp_pic_bboxes(img, bboxes, horizon=horizon)
        elif flag == 4:
            #print('旋转')
