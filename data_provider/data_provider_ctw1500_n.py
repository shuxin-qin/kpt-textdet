...
def load_annoatation_xml(xml_file):
...
def gen_masks(polygons, shape, tags, num):
    
    kernels = []
    for i in range(num):
        kernel = np.zeros(shape, dtype='float32')
        kernels.append(kernel)
    #cent_mask = np.zeros(shape, dtype='uint8')
    ...
    for polygon in polygons:
        ...
        cv2.drawContours(gt_text, [polygon], -1, i + 1, -1)
        if not tags[i]:
            cv2.drawContours(training_mask, [polygon], -1, 0, -1)

        mask = (mask * 0).astype('uint8')
        #mask = np.where(mask>0, 0, 0)
        cv2.drawContours(mask, [polygon], -1, 1, -1)
        #mask, label = cv2.distanceTransformWithLabels(mask, distanceType=cv2.DIST_L2, maskSize=3, labelType=cv2.DIST_LABEL_PIXEL)
        #start = time.time()
            ...
            try:
                ...
                if text_polys.shape[0] > 0:
                    ...
                    for i in range(text_polys.shape[0]):
                        cv2.drawContours(gt_text, [text_polys[i]], -1, i + 1, -1)
                        if not tags[i]:
                            cv2.drawContours(training_mask, [text_polys[i]], -1, 0, -1)
                
                gt_text[gt_text > 0] = 1
               
                gt_center = np.zeros(im.shape[0:2], dtype='uint8')
                kernal_bboxes = shrink(text_polys, 0.4)
                for i in range(text_polys.shape[0]):
                    cv2.drawContours(gt_center, [kernal_bboxes[i]], -1, 1, -1)

                imgs = [im, gt_text, training_mask, gt_center]

                imgs = random_horizontal_flip(imgs)
                imgs = random_rotate(imgs)
                imgs = random_crop(imgs, (input_size, input_size))

                img, gt_text, training_mask, gt_center = imgs[0], imgs[1], imgs[2], imgs[3]
