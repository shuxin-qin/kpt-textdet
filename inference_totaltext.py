...
def inference():
    ...
    ckpt_path='./checkpoint/totaltext/weight50_totaltext-41800'
    sess = tf.Session()
    ...
    F = model_n.model(inputs, is_training=False)
    ...
    # Wt = tf.where(text >= 0.4, x=one, y=zero)
    # hm_hp = hm_hp * Wt

    # det = decode_text(hm, wh, hm_hp, hp_kp, kp_wid, K=100)

    img_folder = 'data/total_text/test/images1'
    out_path = 'data/total_text/test/out1'
    ...
    for i, img_name in enumerate(img_names):
        ...
        results = sess.run(F, feed_dict={inputs: image_data})
        t1 = time.time()
        ...
        print('No.%d Inferencce took %.1f ms- %.1fms (%.2f fps)' % (i, (time.time()-t0)*1000, (time.time()-t1)*1000, 1/(time.time()-t0)), img_name)
        '''
        polygons = []
        if cfgs.use_nms:
            results = []
            ...
            if len(results) > 0:
                ...
                #print(kp_wids)
                #print(bboxes)
                polygons, ilist = text_draw_on_img(original_image, scores, bboxes, kps, kp_wids, thickness=2, drawkp=True)
        ...
        for i in range(len(ilist)):
            ...
            i_mname = os.path.join(out_path, img_name.replace('.jpg', '_00' + str(i) + '.jpg'))
            cv2.imwrite(i_mname, ilist[i])
            print(i_mname)
        ...
        hname = os.path.join(out_path, img_name.replace('.jpg', '_01.jpg'))
        cv2.imwrite(hname, hmm)
        h_mname = os.path.join(out_path, img_name.replace('.jpg', '_02.jpg'))
